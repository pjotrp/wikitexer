...
module DraftModeFormatting
  ...
  def DraftModeFormatting::markup paragraph, creator
    if not $style[:final]
      ...
      paragraph.replace_once('((,\s+(because|while|partly)))', proc { | buf, orig | creator.correct(buf) } )
      ...
      # follow by a comma
      paragraph.replace_once('(((In addition|Likewise|Sometimes|Here|For data)\s))', proc { | buf, orig | creator.correct(buf) } )
      # should have dash
      paragraph.replace_once('((\s(cross (language|platform)|(high|low) performance|\w+ level|\w+ sized)))', proc { | buf, orig | creator.correct(buf) } )
      # should have no dash
      paragraph.replace_once('((\s(open-source)))', proc { | buf, orig | creator.correct(buf) } )
